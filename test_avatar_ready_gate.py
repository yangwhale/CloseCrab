...
改道给数字人 —— 但那一刻 worker 还没进房。那段窗口里说的每个字都写进一条
**对端不存在**的字节流：不报错、不重试，直接没了。
Chris 2026-09-19：「开了 Avatar 以后没有任何的图像和音频。」
**两样同时没有是同一个原因**：没图是 worker 还没发轨，没声是音频已经改道
给了那个还没起来的它。

所以后台等它进房（实测 1.2–1.6 s）再切，进房后再稳半秒。
...
服务端 API 实测坐实：那 45 s 里 `cc-avatar-principal` 的 `tracks=0`；
worker 日志那句「音视频轨已发布」只花 1 ms —— 是 `_lazy_publish`
把发布跳过去了，不是真发了（那句话已经改掉）。**别再拿轨当判据。**

...
4. 大活儿 → **调 `ask_{bot}` 真派出去**，不是嘴上说「该由它做」

第 4 条 2026-09-14 改过。原来写的是「说该由它做，但不要假装已经派出去了」，
理由是那时候**真的没有派活的通道**。现在有了：`ask_{bot}` 走 Firestore inbox，
本体的回复经 `livekit_out` 那条常驻音轨**回到同一个房间里**用它自己的声音说出来。
所以「让用户自己去飞书说」现在是个错误答案 —— 等于把人从语音里赶出去。

配套的两处，缺一条这个功能就是半截的：

- `bots/{bot}.channels.livekit.enabled = true` —— 没开的话本体没有嘴，
  派出去的活会石沉大海（用户听到「已经交给它了」，然后再没下文）。
- 助手**听不见**本体那条轨（混音池按 kind 过滤掉 agent）。这是故意的，
  否则本体一停嘴它就接话，用户听见两个 bot 互相捧哏。代价是它不知道本体
  刚说过什么，用户追问「刚那第二条再讲讲」它是真不知道。

...
**②b ⭐⭐⭐ 配色用 manim 自带的原生那套，不要自己配。**（2026-09-19 现场拍板）
...
| | 短循环片（≤20 秒） | 长叙事片（>20 秒） |
|---|---|---|
| 页面标签 | `autoplay loop muted playsinline` | `autoplay muted playsinline controls`（**不要 loop**） |
| 首尾 | **必须同帧** | 不要求 |
| baseline | 正常登记 | 写 `"loop": false`，守卫跳过首尾检查 |

⭐ 判据：**守卫要守的是「承诺」，不是「形状」。** 片子承诺了 loop 才查首尾；
强行要求长片首尾同帧，等于逼它在结尾把内容全撤掉。

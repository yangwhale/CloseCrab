...
REGION="us-central1"
WEB_ROOT="${CC_PAGES_WEB_ROOT:?Set CC_PAGES_WEB_ROOT env var}"
URL_PREFIX="${CC_PAGES_URL_PREFIX:?Set CC_PAGES_URL_PREFIX env var}"
OUTPUT_DIR="${WEB_ROOT}/assets/veo"
BASE_URL="${URL_PREFIX}/assets/veo"
...
if [[ ${#REF_IMAGES[@]} -gt 3 ]]; then
    echo "Error: Maximum 3 reference images allowed." >&2
    exit 1
fi
...
# Reference images are passed as remaining args
ref_paths = sys.argv[9:]
...
# Reference images for visual guidance (asset type)
if ref_paths:
    instance['referenceImages'] = [
        {'image': encode_image(p), 'referenceType': 'asset'}
        for p in ref_paths
    ]
...
" "$PROMPT" "$COUNT" "$ASPECT" "$DURATION" "$RESOLUTION" "$ENHANCE" "$NEGATIVE" "$IMAGE_PATH" "${REF_IMAGES[@]}")

...
- bot 自查发现 **MEMORY.md 字符数 > 23.5K**（这是真正先到顶的那条线，见下）
  或 sub-file 数 > 100
...
5. **两条上限，字符那条先到 —— 别只盯行数**
   - 字符 **24.4K**（CC CLI 按字符截断，超出部分静默丢弃）← **真正的约束**
   - 行数 **200**（CC CLI 只 load 前 200 行）
   2026-08-10 实测：整理后 176 行 / 23.4K 字符 —— 行数余量 12%，字符余量只有 4%。
   按日常写入速度，**字符大概 3-5 条新条目就顶到，行数还早得很**。
   所以「> 195 行」这个旧触发条件在字符先到顶的情况下**永远不会先触发** ——
   等它响的时候，尾部内容早就被静默截掉了。

...
## O10: Context Usage 上报修复（Critical）

### 依据

Control Board 显示 Bunny context usage 220%（2.2M / 1M）。调查发现两个 bug：

1. **Token 累加 vs 快照**：`_post_message()` 用 `+=` 累加每步的 token 数，但 Kilo 的 `info.tokens`
   是 per-step 快照（`processor.ts:456` 用 `=` 赋值，非累加）。每步返回的是当前 session 总量，
   用 `+=` 会重复计算。ClaudeCodeWorker 正确使用 `=`（line 721）。

2. **Output tokens 不算上下文**：`get_context_usage()` 把 `output_tokens` 也算进 `total_context_tokens`，
   但 output tokens 不占 context window。ClaudeCodeWorker 正确排除了 output（line 761）。

### 步骤

1. `_post_message()` token 字段从 `+=` 改为 `=`（保留 `cost_usd` 的 `+=`，因为 cost 是增量）
2. `get_context_usage()` 的 total 计算去掉 `output_tokens`

### 验证

- 重启 Bunny → Control Board context usage 从 220% 降到正常范围（~20-30%）
- 多轮对话后 context usage 单调递增（不跳跃）

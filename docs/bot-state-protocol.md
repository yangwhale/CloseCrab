...
| `act` | string | 主 agent 此刻在调哪个工具（**过程，不是任务**） |
| `task` | string | 这一轮**被派去干什么** —— 用户原话，截 80 |
| `sum` | string | 这一轮**做成了什么** —— 回复第一句，截 80。干完才有 |
...
### 主 turn 也有任务和摘要（2026-09-20 补）

Chris 的原话：「这个主 agent 为什么不能拥有 Task 和 Summary？后面总是跟着
一句『在跑命令』之类的，那也看不出来这个任务到底在干啥。」

**不是做不到，是之前没送。** 子 agent 的这两样来自派活描述和回报摘要，
主 turn 的对应物一直都在手上：

    任务 = 用户原话（去掉 `[channel:...]` `[当前时间:...]` 那些前缀）
    摘要 = 回复的第一句（**不做二次总结** —— 那要再调一次模型，
           为一行状态不值；第一句通常就是结论）

界面上的规则跟子 agent 那几行**完全一致**：
跑着看 `task`，干完看 `sum`，`act` 只在两者都空时兜底。

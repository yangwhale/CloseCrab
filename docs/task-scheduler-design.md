...
**这是调研里两个 agent 都没提、但会直接决定可用性的问题。**
...
1. **成本方案**（§7.2）：廉价探针 + 升级，还是指数退避，还是两者叠加？**影响架构**。
2. **session 复用策略**（§6.1）：同一 Task 多次触发，复用 session 还是每次全新？还是按 kind 区分？
3. **`every` 默认锚点**（§5.5②）：`complete` 还是 `fire`？
4. **执行历史存哪**（§9.3）：新子集合还是复用 `bots/{name}/logs`？
5. ~~**现存 cron job 的时区迁移**~~ —— **已查清**（§5.5①）：只有 2 条在跑，都是周一晨扫类，且当前语义正确。迁移时表达式与 `tz` 字段必须原子同改。不构成阻塞。
6. **Task 能不能挂多个 Job**：本设计限定 0..1。是否有真实场景需要多个（例如「工作日早 8 点 + 每 30 分钟」双触发）？
7. **时区支持范围**（写 §14.1-B 测试用例时发现的）：只支持 `Asia/Hong_Kong` + `UTC`（传别的直接报错），还是支持任意 IANA 时区（那就必须处理 DST 的两个恶心 case：春季不存在的挂钟时刻、秋季重复的挂钟时刻）？
   → 倾向**先只支持两个**，把 DST 挡在门外，等真有跨时区需求再开。但这个决定要写进文档，否则将来有人传了 `America/New_York` 会得到静默错误的行为。
...
**P4 — `every` kind + 锚点/补跑语义**
- 严格说 P0 之后就能用 cron 凑合表达「每 30 分钟」（`*/30 * * * *`），但那是挂钟对齐的，没有 anchor 语义。P4 补上。

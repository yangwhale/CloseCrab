...
> **已修**：`cron-tool.py` 现在 `DEFAULT_TZ = "Asia/Hong_Kong"`（:120），
> `--tz` 只接受 `Asia/Hong_Kong` / `UTC` 两个值，传别的直接报错（:133）；
> 另有 `migrate-tz` 子命令原子改写历史任务的表达式和时区。
> **`--cron "0 8 * * *"` 现在就是香港时间早 8 点**，不需要自己换算。

**G5 — 无成本控制。**
每次触发 = 一次完整 LLM turn。「每 3 分钟查一次」= 一天 480 次。详见 §7。

**G6 — 多 daemon 并发无锁（2026-08-08 实施 P0 时发现，已修）。**
`cron-daemon` 同时跑在 **cc-tw / glinux / glinux_bj 三台**，扫同一张 `scheduled_jobs`，既无 leader 选举也无锁。旧的「读 `fire_at` → 判断到期 → 写新 `fire_at`」在两个 daemon 同秒 tick 时会双双命中同一条 job：一次到期，两条 inbox，两个完整 LLM turn。

翻了 118 条历史 inbox 消息**没有观测到真实重复**——竞态窗口约是 30 秒周期里的 1 秒，样本太小还没撞上。**但那是运气不是设计。**

顺带扫到一条佐证：`2026-07-26 03:41:33` 那一秒三条 job 同时触发，其中两条周任务当天是周日——那是 daemon 停机后的**补跑风暴**，给 §5.5③ 的补跑策略提供了实证。
...
### 6.0.1 共享清单的边界：动作跟机器有没有关系

> **2026-08-08 Chris 提问**：「三台机器怎么能盯着一个表呢？彼此有影响。
> 那就是每台机器盯自己的表就好了吧？你这台机器上明明没有那几个 bot，
> 你还盯着人家去执行这个任务。」

这个担心是对的，但**只对其中一张表成立**。判据是一句话：
**到期那一刻，daemon 做的动作跟它跑在哪台机器上有没有关系？**

| | 到期动作 | 跟机器有关？ | 结论 |
|---|---|---|---|
| `scheduled_jobs`（定时提醒） | 往 Firestore 写一条 inbox | **无关** | 共享，任一 daemon 都行 |
| `watch_tasks`（盯任务） | 在**本机**起 agent 跑 shell、读**本机**文件 | **强相关** | 必须钉在创建它的机器上 |

**为什么 cron 那张表不该按机器切**：目标 bot 是靠自己的 `on_snapshot` 监听
inbox 的，它在哪台机器上跟谁写这条消息完全无关。切开反而**丢掉冗余** ——
cc-tw 挂了，钉在 cc-tw 上的提醒就再也不会响。共享的话另外两台会顶上。

**为什么 watch 那张表必须切**：探针 prompt 通常长这样「读 `/tmp/t80.log`
判断训练进度」。glinux 的 daemon 抢到一条 cc-tw 创建的任务，会对着一个
**不存在的路径**跑 agent —— 然后一直返回 SKIP。**抢占是原子的、不会重复
执行，但执行落在了错的机器上**，这不是锁能解决的问题，而且完全静默：
没有报错，只是那个任务永远盯不到东西。

**落地方式**：不是建三张表，而是给 `watch_tasks` 加一个 `host` 字段
（创建时记 `socket.gethostname()`），tick 时只处理自己那些。这样
「一张全局清单」的心智不变（`list` 仍能看到全部、跨 bot 写仍然可行），
只是**执行被钉住**。

**一个实现上的细节，写错了会静默出事**：host 判断必须在 `_claim` **之前**。
先 claim 再发现不是自己的，`next_fire_at` 已经被推到下一个周期了 ——
真正该跑的那台机器这一轮就被饿死，而且没有任何迹象。测试
`test_host_filter_runs_before_claim` 锁住这条。
...
### 6.4 多 daemon 的事务抢占（修 G6）

不做「只留一台 daemon」，也不做「三台都升级就完事」——前者丢冗余，后者只是把竞态窗口缩小。**正确做法是让多 daemon 在设计上就安全**：

在一个 Firestore 事务里完成「读 `fire_at` + 校验到期 + 推进 `fire_at`」，claim 成为串行化点。输的一方重读到已推进的值就跳过，`tick` 输出里记一条 `lost_race`。

这样多 daemon 从隐患变成**冗余优势**：没有哪台机器是特殊的，挂一台不影响排期。

...
from ..utils.model_display import shorten_model_name as _shorten_model_name
...
_MODEL_ALIASES: dict[str, tuple[str, str]] = {
    ...
    "O50": ("claude-opus-5", "Opus 5.0, 1M ctx"),
    "O55": ("claude-opus-5-5", "最新 Opus 5.5, 1M ctx"),
...
class FeishuChannel(Channel):
    ...
    async def _tts_and_send_one(self, chat_id: str, text: str) -> bool:
        ...
        try:
            ...
            proc = await asyncio.create_subprocess_exec(
                "python3", tts_script, text, "--voice", tts_voice(),
    ...
    async def _send_voice_summary(self, chat_id: str, text: str):
        ...
        if not heard_live:
            ...
            log.info(f"Voice summary via streaming (ogg skipped) to {chat_id}")
            ...
            "elements": [
                {
                    ...
                    "fields": [
                        ...
                        {"is_short": True, "text": {"tag": "lark_md", "content": f"**Model:** {_shorten_model_name(info.get('backbone_model', '')) or '?'}"}},
    ...
    def _fmt_progress_note(played: float, total: float, active: bool) -> str:
        ...
        ⚠️ **总长未知时绝不能编一个分母出来。** 播放器在生成阶段会把 total 报 0
        (见 player._report), 因为那时的「已落盘长度」不是最终长度。这里要如实
        显示「还在生成」—— 2026-09-17 就是因为卡片显示 `15/16s`, 看着像播完了,
        用户按了重播, 连锁触发 replay 冻结直播段, 整条回复只剩前 16 秒。
        既然分母是假的, 就别显示分母。
        """
        if total <= 0:
            # 仍然给出已播秒数 —— 只显示一个省略号看不出它到底动没动。
            return f"{played:.0f}s ⏳ 生成中" if played >= 1 else "⏳ 生成中"
        ...
        if self._livekit_config.get("enabled"):
            try:
                ...
                self._voice_io = LiveKitVoiceIO(
                    ...
                    stt_phrase_boost=bool(self._livekit_config.get("stt_phrase_boost", False)),
                )

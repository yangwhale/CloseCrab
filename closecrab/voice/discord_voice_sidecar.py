...
_funasr_text_buffer = []  # FunASR offline 结果 buffer, PTT 松手时一次性提交给 LLM
...
def _funasr_send_start_of_speech():
    ...
    if _funasr_speaking:
        return
    _funasr_text_buffer = []  # 新一轮说话，清空上次残留
    ws = _funasr_ab_init()
    if ws is None:
        return
    try:
...
def _on_discord_speaking_stop():
    """PTT 松手 (Opcode 5 speaking=0)：flush FunASR buffer → 送 LLM。"""
    ...
    _funasr_text_buffer = []
    if not buf:
        log.info("[FunASR] PTT 松手但 buffer 空 (可能说话太短)")
        return

    combined = "".join(buf).strip()
    if not combined:
        return
    log.info("[FunASR] PTT 松手 → flush %d 段 buffer → LLM: %s", len(buf), combined[:80])

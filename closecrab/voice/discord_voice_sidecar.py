...
_EMOTION_INSTRUCT_MAP = {
    ...
    # ⚠️ 原来这里有一条 "whispers"（「音量: 较小，悄悄话」）—— 删了。
    #    那正是「听不见」的病根，而且这个标签现在会在 normalize_tts_tags
    #    里被换成 [casually]，根本到不了这张表。缺失时 .get(tag, "") 回落
    #    到中性指令，是安全的默认。
...
async def _do_speak(text: str, fid: str = "", backend: str = ""):
    ...
    **`fid` 有没有决定走哪一档**（2026-09-14 定）：

    - 有 fid = 一条完整回复。落盘、可回放、卡片上那五个按钮和进度条都认它。
    - 没有 fid = 工具调用时「滋」的那一声。只出声：内存里过一遍，不落盘、
      不上报位置。一句话几秒长，给它配一条跟完整回复一样的状态栏只会把卡片
      搞乱；不上报位置之后，按钮和进度条自然抓不到它，不用另加开关。

    两档共用同一个播放器、同一条队列，所以提示音和回复永远不会同时出声。
    """
    import time as _time
    global _tts_interrupted
    from . import playback
    from .gemini_tts import normalize_tts_tags

    # 把「会让人听不见」的标签换掉。**必须在这里也做一次** ——
    # qwen3 那条路下面直接拿 `text` 去 `_split_by_emotion`，不走
    # `_clean_text_for_tts`，只在那边挂等于漏掉一整个后端。
    ...
    if not live:
        # 走到这儿只剩一种情况：**fid 非空但不合法**（含路径分隔符之类）。
        # 空 fid 是合法的，那是中间过程提示音那一档，`begin` 会开一条
        # 不落盘、不上进度条的轨 —— 09-13 到 09-14 之间它被当成「开播失败」
        # 一路丢掉，工具提示因此在三个出口上全哑了。
        #
        # 仍然不绕过播放器偷偷播：那样位置又会分叉，正是统一播放器要治的病。

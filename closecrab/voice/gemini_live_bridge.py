...
_TOOL_ASK_OWNER = types.FunctionDeclaration(
    ...
    description=(
        ...
        "\n\n"
        f"**两条独立的触发条件，满足任意一条就调：**\n"
        f"（一）能力不够：会改变系统状态的动作、要好几分钟的调研、要动脑的方案设计。\n"
        f"（二）**用户点了「{BOT_NAME}」的名字** —— 不管后面跟的是什么，"
        f"「让{BOT_NAME}继续」「问{BOT_NAME}…」「告诉{BOT_NAME}…」「叫{BOT_NAME}…」"
        f"「请{BOT_NAME}帮忙…」「跟{BOT_NAME}说…」都算。这条**不看内容难不难**："
...
_PERSONA_FALLBACK = (
    ...
    f"你有 run_shell 查这台机器的状态（只读），"
    f"有 ask_{BOT_NAME} 把干不了的活交给 {BOT_NAME}（发出去就返回，它自己会说结果）。"
)

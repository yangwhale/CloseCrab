...
#
# ---- 上线当晚实测，**我上面那条预测是错的，反了** ----
# 探针（生产配置 + 文本输入，要出处 / 一般时事各 3 遍）：
#   第一版描述 → search_web **6 次一次都没被选中**。要链接那题它要么用内置
#   搜索、要么干脆派给 bunny。对比台那条「摆着就会自发选它」在这里不成立 ——
#   那台上**没有 ask_<bot>、也没有这份前台 persona**。这边 persona 通篇在说
#   「默认动作是派活」，我又在描述里写了一堆「别用它」，两股力叠起来直接把它
#   压死了。**同一个工具，换个上下文，结论整个反过来。**
# 于是补了一条具体的、可判定的理由（不是加强语气）：**内置搜索给不出真网址**。
# 它回的是 vertexaisearch.cloud.google.com/grounding-api-redirect/… 这种重定向
# 串，又长又念不出来，贴给用户也不落到原网站 —— 这是内置那条**做不到**的事，
# 不是「哪个更好」的偏好问题。
#   改完 → 要出处 3/3 调 search_web，拿回真实 starferry.com.hk 链接；
#          一般时事 3/3 仍然走内置、没被抢路；6/6 都先说了「我搜一下啊」。
#          静音实测 1.5 / 2.1 / 6.7 秒。
# 教训：想让模型选一个工具，给它**一件另一个工具办不到的事**，
# 比在描述里把话说重有用得多。
...
_TOOL_SEARCH_WEB = types.FunctionDeclaration(
    ...
    description=(
        ...
        "\n\n"
        "**用户要链接的时候必须用这个，不能用你自带的搜索。** "
        "你自带那个回的网址是一串重定向地址"
        "（vertexaisearch.cloud.google.com/grounding-api-redirect/…），"
        "又长又念不出来，贴给用户也不是原网站。真网址只有这个工具给得了。\n"
        "**其它该用它的时候：** 用户要出处、要「你从哪看到的」；"
        "或者你想确认一个具体网页上写了什么。\n"

...
```
                              ...
                              │                    GCS Bucket (共享层)                       │
                              │  ┌──────────────┐  ┌──────────────┐  ┌──────────────────┐   │
                              │  │ memory/shared │  │  cc-pages/   │  │  data-exchange/  │   │
                              │  │  公共知识库   │  │  报告/文档   │  │ 模型/数据/结果   │   │
                              │  └──────┬───────┘  └──────┬───────┘  └────────┬─────────┘   │
                              │         │                 │                   │              │
                              └─────────┼─────────────────┼───────────────────┼──────────────┘
                                        │ gcsfuse         │ gcsfuse           │ gcsfuse
                    ┌───────────────────┼─────────────────┼───────────────────┼──────────┐
                    │                   │                 │                   │          │
          ┌─────────▼─────────┐ ┌──────▼────────┐ ┌─────▼──────────┐ ┌─────▼──────────┐
          │   Leader (Jarvis) │ │ Teammate Tommy│ │ Teammate Hulk  │ │ Teammate Bunny │
          │   gLinux / Mac    │ │ TPU v6e-8 VM  │ │ gLinux (GPU)   │ │ B200 x8 VM     │
          │                   │ │               │ │                │ │                │
          │ • 任务拆解/派活   │ │ • JAX 训练    │ │ • 数据分析     │ │ • vLLM 推理    │
          │ • 汇总报告        │ │ • MaxText     │ │ • 基准测试     │ │ • sglang 服务  │
          │ • 用户交互        │ │ • 模型评估    │ │ • 代码审查     │ │ • 模型对比     │
          └────────┬──────────┘ └───────┬───────┘ └───────┬────────┘ └───────┬────────┘
                   │                    │                 │                  │
                   │      Firestore Inbox (on_snapshot 实时推送)             │
                   └────────────────────┴─────────────────┴──────────────────┘
```
...
```
用户                    Leader (Jarvis)          Tommy (TPU)            Bunny (B200)
  │                        │                        │                      │
  │  "用 Llama 4 跑训练    │                        │                      │
  │   和推理 benchmark"    │                        │                      │
  ├───────────────────────>│                        │                      │
...
```bash
# 1. 前提：GCE VM + service account 有 cloud-platform scope
# 2. 前提：Firestore 数据库已创建，config/secrets 已有 API keys

...
CloseCrab 将 AI CLI 工具包装为多平台聊天 Bot。默认通过 Unix socketpair 与 Claude Code CLI 通信，也支持通过 ACP 协议驱动 Gemini CLI 或 OpenClaw Gateway——在聊天平台里提供完整的 AI Agent 体验：工具调用、MCP Server、Skills、Auto Memory、Agent Teams，一个不少。内置基于 [Karpathy LLM Wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f) 理念的个人知识 Wiki 系统，让知识在对话中持续编译和增值。
...
OpenClaw Worker 通过外部 Gateway 管理模型调用和 MCP 插件，适用于需要 Gemini Flash 等模型的场景。

...
> 把 Claude Code / Gemini CLI 的全部能力接入 Discord / 飞书 / Lark / 钉钉，打造你的 24/7 AI 助手团队。

CloseCrab 将 AI CLI 工具包装为多平台聊天 Bot。默认通过 Unix socketpair 与 Claude Code CLI 通信，也支持通过 ACP 协议驱动 Gemini CLI——在聊天平台里提供完整的 AI Agent 体验：工具调用、MCP Server、Skills、Auto Memory、Agent Teams，一个不少。内置基于 [Karpathy LLM Wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f) 理念的个人知识 Wiki 系统，让知识在对话中持续编译和增值。
...
- **Worker** — 与 AI CLI 进程通信的抽象层，支持两种实现：
...
| | ClaudeCodeWorker | GeminiACPWorker |
|---|---|---|
| **CLI** | Claude Code (`claude`) | Gemini CLI (`gemini`) |
| **通信协议** | Unix socketpair IPC | ACP (JSON-RPC 2.0 / NDJSON) |
| **进程模型** | 持久子进程 | 持久子进程 |
| **MCP 加载** | 自动读取 `~/.claude.json` | 需在 `session/new` 显式传入 |
| **内置搜索** | WebSearch (Brave) | Google Search (Grounding) |
| **Google 内部工具** | — | Extensions (Workspace/CodeSearch/...) |
| **配置切换** | `set-worker-type <bot> claude` | `set-worker-type <bot> gemini` |
...
两种 Worker 均支持交互式流程：`ExitPlanMode`（Plan 审批）、`AskUserQuestion`（多选项回复）。
...
| `worker_type` | — | Worker 类型：`claude`（默认）或 `gemini` |
...
BotCore 内部维护一个 `_workers: dict[str, Worker]` 字典，以用户 ID 为 key（Discord user ID / 飞书 open_id），每个用户对应一个独立的 Worker（`ClaudeCodeWorker` 或 `GeminiACPWorker`，取决于 `worker_type` 配置）。用户首次发消息时自动创建 worker，启动一个独立的 CLI 子进程。

```
Alice (Discord)  ──→  BotCore._workers["alice"]  ──→  claude 进程 A (Session abc123)
Bob   (飞书)     ──→  BotCore._workers["bob"]    ──→  gemini 进程 B (Session def456)

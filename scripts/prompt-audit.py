...
#
# ⚠️ 这个脚本有**两个调用方**，加历史时差点漏掉第二个：
#   1. 周一 cron 定时体检           → 要写历史，那才是趋势
#   2. main.py 每个 bot 启动时后台跑 → **必须 --no-save**
# 第二个的目的是往 bot.log 留一份当下快照给人看，不是记趋势。
# 少了 --no-save，一台机器 5 个 bot 每次重启就写 5 条，
# 周一那次的「上次」会变成几分钟前某个 bot 重启的记录 —— 环比退化成跟噪音比。
# 教训：给一个脚本加副作用（写文件）之前，先 grep 清楚谁在调它。
